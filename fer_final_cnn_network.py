...
# performing histogram equalization on the normalized data
img_array = img_array / 255.
histogram_array = np.cumsum(img_array)
transform_map = np.floor(255 * histogram_array).astype(np.uint8)
img_list = list(img_array.flatten())
eq_img_list = [transform_map[p] for p in img_list]
img_list_v = list(X_valid.flatten())
img_array = np.reshape(np.asarray(eq_img_list), img_array.shape)
